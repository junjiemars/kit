...
if [ -n "$ngx_target" ]; then
	...
	for i in "${!NGX_TARGET[@]}"; do
		if [ "$ngx_target" = "${NGX_TARGET[$i]}" ]; then
			NGX_TARGET_IDX=$i			
			break
		fi
	done

	if [ -z "$NGX_TARGET_IDX" ]; then
...
gen_http_conf() {
	local ngx="nginx.conf"
	local old="nginx.conf.b0"
...
case "$command" in
  ...
  configure)
	...
	shell)
		...
		case "${NGX_TARGET[${NGX_TARGET_IDX}]}" in
			http) gen_http_conf ;;
