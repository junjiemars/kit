...
=~/Library=
#+BEGIN_SRC sh
rm -r ~/Library/Saved Application State/xxx
rm -r ~/Library/Containers/xxx
rm -r ~/Library/Caches/xxx
rm -r ~/Library/Application Support/xxx
rm -r ~/Library/Application Scripts/xxx
rm -r ~/Library/Preferences/xxx
#+END_SRC
...
#+BEGIN_SRC sh
make -f docker/compose/mysql.Makefile start
#+END_SRC
...
Find the configuration file: =idea64.conf= at =~/.idea/=.
#+BEGIN_SRC sh
-Xms128m
-Xmx1024m
...
- =-XX:MaxPermSize=: the maximum permanent space used by the JVM are
  stored and never deallocated.
