...
comment_backup () {
  ...
  local pre="${f}.pre"
  if [ -f "${ori}" ]; then
    echo "# origin backup: ${ori}"
  fi
  if [ -f "${pre}" ]; then
    echo "# previous backup: ${pre}"
  fi
}
...
# generated by:
#   $SH <($SH_ENV)
# keys: check_*, export_*, install_*, list_*, select_*, make_*
#------------------------------------------------
...
export_lean_elan_path () {
  local d="\$(check_lean_elan_env)"
  if [ -z "\${d}" ]; then
    return 1
  fi
...
make_lean_elan_completion () {
  local c="${r}/_completion"
  local rc="\${c}/_elan"
  elan completions $SH > \$rc
}
...
ISO_VER = \${v}
ISO_URL = \${i}
ISO = ./iso/\${o}_\\\$(MAC)-\\\$(ISO_VER).iso
IMG = ./img/\${o}_\\\$(MAC)-\\\$(ISO_VER).qcow2
IMG_SIZE = 20G
...
help:
	...
	@echo "  download-iso    - download iso image"
	@echo "  create-img      - create disk image"
	@echo "  start           - start image"
	@echo "  clean           - clean disk image"
