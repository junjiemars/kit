...
if [ ! -f "${inc_bat_sh}" ]; then
  echo "! ${inc_bat_sh} no found"
  return 1
fi

msvc_path=\$(echo "\`${inc_bat} \$@\`" | sed -e 's#\"##g')

inc_lns=
IFS=\$';'
for i in \`echo "\${msvc_path}"\`; do
  ln=\$(echo "\\\\\${i}" | sed -e 's#^\\\\\\([a-zA-Z]\\):\\\\#\\\\\\l\\1\\\\#' -e 's#\\\\#\\/#g')
  inc_lns=\${inc_lns}\${ln:+:\$ln}
done
unset IFS
