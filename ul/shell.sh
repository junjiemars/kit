...
gen_dot_shell_profile() {
	...
	if [ "zsh" = "$SH" ]; then
		profile="$HOME/.zpprofile"
	fi
	echo -n "+ generating $profile ... "
  cat << END > "$profile"
#------------------------------------------------
# target: call $HOME/.${SH}rc
# author: junjiemars@gmail.com
# generated by: $SH <($INSIDE_KIT_SH_ENV)
#------------------------------------------------
...
# ${logout}: executed by ${SH}(1) when login shell exits.
...
gen_dot_shell_profile
gen_dot_shell_logout
gen_dot_shell_rc "$HOME/.${SH}rc"

gen_dot_shell_init "$HOME/.${SH}_init"
